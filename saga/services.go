...
func (s *InventoryService) Reserve(event Event) {
	...
	if s.failForOrders[orderID] {
		...
		s.bus.Publish(Event{Name: "inventory.reserve.failed", SagaID: event.SagaID, Payload: map[string]any{"order_id": orderID, "error": err}})
		return
	}

	s.reservations[orderID] += qty
	s.bus.Publish(Event{Name: "inventory.reserved", SagaID: event.SagaID, Payload: map[string]any{"order_id": orderID, "quantity": qty}})
}
...
func (s *InventoryService) Release(event Event) {
	...
	s.bus.Publish(Event{Name: "inventory.released", SagaID: event.SagaID, Payload: map[string]any{"order_id": orderID, "quantity": qty}})
}
...
func (s *PaymentService) Charge(event Event) {
	...
	if s.failForOrders[orderID] {
		...
		s.bus.Publish(Event{Name: "payment.charge.failed", SagaID: event.SagaID, Payload: map[string]any{"order_id": orderID, "error": err}})
		return
	}

	s.charges[orderID] = amount
	s.bus.Publish(Event{Name: "payment.charged", SagaID: event.SagaID, Payload: map[string]any{"order_id": orderID, "amount": amount}})
}
...
func (s *PaymentService) Refund(event Event) {
	...
	s.bus.Publish(Event{Name: "payment.refunded", SagaID: event.SagaID, Payload: map[string]any{"order_id": orderID, "amount": amount}})
}
...
func (s *ShippingService) Create(event Event) {
	...
	if s.failForOrders[orderID] {
		...
		s.bus.Publish(Event{Name: "shipping.create.failed", SagaID: event.SagaID, Payload: map[string]any{"order_id": orderID, "error": err}})
		return
	}

	trackingID := fmt.Sprintf("TRK-%s", orderID)
	s.shipments[orderID] = address
	s.bus.Publish(Event{Name: "shipping.created", SagaID: event.SagaID, Payload: map[string]any{"order_id": orderID, "tracking_id": trackingID}})
}
