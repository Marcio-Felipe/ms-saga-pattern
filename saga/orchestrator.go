...
func (o *OrderSagaOrchestrator) onInventoryReserved(event Event) {
	...
	o.bus.MustPublish(Event{Name: "payment.charge.requested", SagaID: event.SagaID, Payload: map[string]any{"order_id": event.Payload["order_id"], "amount": amount}})
}
...
func (o *OrderSagaOrchestrator) onPaymentCharged(event Event) {
	...
	o.bus.MustPublish(Event{Name: "shipping.create.requested", SagaID: event.SagaID, Payload: map[string]any{"order_id": event.Payload["order_id"], "address": address}})
}
...
func (o *OrderSagaOrchestrator) onPaymentFailed(event Event) {
	...
	o.bus.MustPublish(Event{Name: "inventory.release.requested", SagaID: event.SagaID, Payload: map[string]any{"order_id": event.Payload["order_id"]}})
}
...
func (o *OrderSagaOrchestrator) onShippingFailed(event Event) {
	...
	o.bus.MustPublish(Event{Name: "payment.refund.requested", SagaID: event.SagaID, Payload: map[string]any{"order_id": event.Payload["order_id"]}})
	o.appendCompensation(event.SagaID, "INVENTORY_RELEASE_REQUESTED")
	o.bus.MustPublish(Event{Name: "inventory.release.requested", SagaID: event.SagaID, Payload: map[string]any{"order_id": event.Payload["order_id"]}})
}
...
func BuildOrchestrator(failInventoryFor, failPaymentFor, failShippingFor map[string]bool, transport Transport) (*OrderSagaOrchestrator, *EventBus) {
	logger := slog.Default()
	bus := NewEventBus(logger, transport)
